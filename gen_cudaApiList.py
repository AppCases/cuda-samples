...
for d in sample_dirs:
    ...
    for sample_name in pathlist:
        if Path(sample_dir + '/' + sample_name).is_dir():
            ...
            entry = root.find('cuda_api_list')

            if (entry):
                entry.clear()
            else:
                entry = ET.SubElement(root, 'cuda_api_list')

            # o = only matching
            # h - no file name
            os.system('cd ' + sample_dir + '/' + sample_name + ' && grep -oshR \"cuda[a-zA-Z]\+[(]\" *.cpp > ./'+sample_name+'toolkit.txt')
            os.system('cd ' + sample_dir + '/' + sample_name + ' && grep -oshR \"cuda[a-zA-Z]\+[(]\" *.cu >> ./'+sample_name+'toolkit.txt')
            os.system('cd ' + sample_dir + '/' + sample_name + ' && grep -oshR \"cuda[a-zA-Z]\+[(]\" src/*.cpp >> ./'+sample_name+'toolkit.txt')
            os.system('cd ' + sample_dir + '/' + sample_name + ' && grep -oshR \"cuda[a-zA-Z]\+[(]\" src/*.cu >> ./'+sample_name+'toolkit.txt')
            ...
            os.system('cd ' + sample_dir + '/' + sample_name + ' && grep -oshR \"cu[A-Z][a-zA-Z]\+[(]\" *.cpp > ./'+sample_name+'driver.txt')
            os.system('cd ' + sample_dir + '/' + sample_name + ' && grep -oshR \"cu[A-Z][a-zA-Z]\+[(]\" *.cu >> ./'+sample_name+'driver.txt')
            os.system('cd ' + sample_dir + '/' + sample_name + ' && grep -oshR \"cu[A-Z][a-zA-Z]\+[(]\" src/*.cpp >> ./'+sample_name+'driver.txt')
            os.system('cd ' + sample_dir + '/' + sample_name + ' && grep -oshR \"cu[A-Z][a-zA-Z]\+[(]\" src/*.cu >> ./'+sample_name+'driver.txt')
